...
async function requestParticipation(electionId, adminEmail) {
    try {
        ...
        showToast(tElector("Demande envoyé"), "success");
        await loadElectorDashboard();
    } catch (error) {
        showToast(tElector("Error"), "error");
    }
}
        ...
        sortedElections.forEach((election) => {
            ...
            card.innerHTML = `
                ...
                <p class="text-sm text-slate-500 mb-2">
                   Administrateur :
                     <span class="font-semibold">${escapeHtml(election.admin_nom || "-")}</span>
                     <span class="text-slate-400">(${escapeHtml(election.admin_email || "-")})</span>
                </p>`
