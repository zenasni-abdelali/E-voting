...
async function submitVoteForCandidate(candidatId, triggerBtn) {
    ...
    try {
        ...
        showToast(tElector("voteAccepted"), "success");
        closeVoteModal();
...
async function handleLogout() {
    ...
    if (!permitted) return;
    const ok = await askConfirmation({ title: tElector("logoutTitle"), message: tElector("Voulez-vous vraiment quitter ?") });
    if (ok) {
...
async function requestParticipation(electionId, adminEmail) {
    try {
        ...
        showToast(tElector("requestSent"), "success");
        await loadElectorDashboard();
    } catch (error) {
